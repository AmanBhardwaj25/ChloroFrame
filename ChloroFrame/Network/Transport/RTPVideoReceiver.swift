...
final class RTPVideoReceiver {
    ...
    func start(host: String, serverPort: UInt16, localPort: UInt16, pingPayload: String,
        ...
        // IPv6 counterpart (level IPPROTO_IPV6 instead of IPPROTO_IP). macOS-only: tvOS has no
        // AWDL competition concern (often Ethernet) and does not do interface discovery (port
        // plan 6.4), so the IP_BOUND_IF lock and NetworkMonitor dependency are dropped there.
        #if os(macOS)
